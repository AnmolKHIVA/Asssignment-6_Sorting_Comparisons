...
//TIME COMPLEXITY OF THIS CODE ( USING BUBBLE SORT ) - O(n^2).
pair<int,int> bubbleSort(int *input, int size)
{
    ...
    pair<int,int> ans;
    ans.first = count_swaps;                      //Number of swaps.
    ans.second = count_comparisons;               //Number of comparisons.
    return ans;
}

//TIME COMPLEXITY OF THIS CODE ( USING SELECTION SORT ) - O(n^2).
...
int main()
{
    ...
    {
        cout << "MORE SWAPS ARE REQUIRED FOR THE BUBBLE SORT ." << endl;
    }
    ...
    {
        cout << "BOTH ALGORITHMS REQUIRE EQUAL NUMBER OF SWAPS." << endl;
    }
    cout << endl;
